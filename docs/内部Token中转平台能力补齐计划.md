...
**执行记录（2026-09-24）：** Task 3.3 已实现，代码、迁移及针对性验证完成，未发布。

- 在模型级新增 `route_strategy`，支持优先级、权重、最低成本、最低延迟四种策略，默认保持原有优先级排序。
- `select_candidates()` 已应用策略；权重策略使用绑定权重进行随机排序，成本和延迟策略使用同模型/渠道近 24 小时成功请求的历史平均值，无历史数据的渠道排在后面。
- 管理端模型配置可选择策略，模型列表及渠道绑定抽屉展示当前策略。
- 已执行 `20260924_1100_model_route_strategy` 迁移，业务库当前版本为该 head。
- 路由策略、路由日志、渠道健康和限流针对性测试共 14 项通过；前端 `npm run build` 通过，未进行浏览器点击验收。
